...
def result():
    ...
    if request.method == 'POST':
        ...
        if int(model_predictor) == 1:
            ...
            with open('pkl_objects/KNNClassifier.pkl', 'rb') as my_model_pkl:
                my_model = pickle.load(my_model_pkl)
           
        if int(model_predictor) == 2:
            mp = 'Naive Bayes Model'
            with open('pkl_objects/NBClassifier.pkl', 'rb') as my_gaumodel_pkl:
                my_model = pickle.load(my_gaumodel_pkl)
        if int(model_predictor) == 3:
            mp = 'Neural Network Model'
            with open('pkl_objects/ANNClassifier.pkl', 'rb') as my_nnmodel_pkl:
